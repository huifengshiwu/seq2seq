...
./seq2seq.sh ${model_dir}/MT.1/config.yaml --eval ${eval_dir}/$1.ASR.1.greedy.out ${data_dir}/$1.fr -v --output ${eval_dir}/$1.ASR.1.MT.1.cascaded.greedy.out --gpu-id ${gpu_id}
./seq2seq.sh ${model_dir}/MT.1/config.yaml --eval ${eval_dir}/$1.ASR.1.beam8.out ${data_dir}/$1.fr -v --beam-size 8 --output ${eval_dir}/$1.ASR.1.MT.1.cascaded.beam8.out --gpu-id ${gpu_id}
./seq2seq.sh ${model_dir}/MT.1/config.yaml --eval ${eval_dir}/$1.ASR.1-2.ensemble.beam8.out ${data_dir}/$1.fr -v --beam-size 8 --ensemble --checkpoints ${model_dir}/MT.{1,2}/checkpoints/best --output ${eval_dir}/$1.ASR.1-2.MT.1-2.cascaded.ensemble.beam8.out --gpu-id ${gpu_id}
}
