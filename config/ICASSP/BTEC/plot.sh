#!/usr/bin/env bash
model_dir=models/ICASSP/BTEC.clean
...
cat ${model_dir}/AST.3/ASR.log | grep "step " | cut -d' ' -f5,5 | awk '{ print $1 + 60000 }' > ${eval_dir}/ASR.multitask.steps

cat ${model_dir}/AST.3/MT.log | grep "dev score=" | cut -d' ' -f5,5 | cut -d'=' -f2,2 > ${eval_dir}/MT.multitask.scores
cat ${model_dir}/AST.3/MT.log | grep "step " | cut -d' ' -f5,5 | awk '{ print $1 + 100000 }'  > ${eval_dir}/MT.multitask.steps
...
cat ${model_dir}/AST.3/AST.log | grep "dev score=" | cut -d' ' -f5,5 | cut -d'=' -f2,2 > ${eval_dir}/AST.3.scores
cat ${model_dir}/AST.3/AST.log | grep "step " | cut -d' ' -f5,5 > ${eval_dir}/AST.3.steps
