...
def attention_decoder(decoder_inputs, initial_state, attention_states, encoders, decoder, encoder_input_length,
    ...
    def target_look(states, state, time):
        length = tf.expand_dims(time, axis=0)
        length = tf.tile(length, [batch_size])
        context, _ = global_attention(state, hidden_states=states, encoder_input_length=length,
                                      encoder=decoder, scope='target_attention')
        return context
    ...
    padding = tf.zeros([batch_size, time_steps, state_size])
    state_acc = tf.concat([tf.expand_dims(initial_state, axis=1), padding], axis=1)
    ...
    def _time_step(time, input_, input_symbol, pos, state, output, outputs, states, weights, attns, prev_weights,
        ...
        if decoder.trg_attention:
            # TODO: try with input_ instead of state
            # TODO: try using ctx to compute context or conversely
            trg_context = target_look(state_acc, output, time + 1)
            context = tf.concat([context, trg_context], axis=1)
        ...
        state_acc = tf.concat([state_acc[:,:time + 1], tf.expand_dims(state, axis=1), state_acc[:, time + 2:]],
                              axis=1)
    ...
    with tf.variable_scope('decoder_{}'.format(decoder.name)):
        _, _, _, new_pos, new_state, _, outputs, states, weights, attns, new_weights, samples, _ = tf.while_loop(
            ...
            loop_vars=(time, initial_input, initial_symbol, initial_pos, initial_state, initial_output, outputs,
                       weights, states, attns, initial_weights, samples, state_acc),
